...
for ss = all
    ...
    load(filetag) %, 'dataHER_LCMV', 'time_HER_ms', 'Latkeep_sample', 'Latkeep_ms', 'latency_tpeaks', 'latency_tpeaks_sample')
    ...
    analysis_window = [0.1, 0.3]; % period of interest: 0.1 to 0.4 seconds

    % Ensure `time` variable matches the epoch length
    epoch_length = size(dataHER_LCMV, 3); % Assuming datasrc contains data in [label, trial, time] format
    ...
    HEPnorm = nan(size(dataHER_LCMV)); % Same size as datasrc

    % Loop through each label and trial
    for l = 1:size(dataHER_LCMV, 1) % For each anatomical label
        for t = 1:size(dataHER_LCMV, 2) % For each trial
            % Extract single trial HEP
            HEP = squeeze(dataHER_LCMV(l, t, :));
    ...
    % Compute mean normalized HEP in the analysis window (0.1-0.3 s)
    mean_HEPnorm = nan(size(dataHER_LCMV, 1), size(dataHER_LCMV, 2)); % Preallocate for mean HEPnorm
    ...
    HGF = alltrials(indfinal_fin);
    alltrials1 = est.sahat(:,1);
    HGF1 = alltrials1(indfinal_fin);
