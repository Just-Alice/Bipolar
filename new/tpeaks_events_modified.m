...
for n = all
    ...
    event = ft_read_event(fif_filename, 'chanindx', channel);   % stimulus channel has to be entered for each participant specifically

    %% All events b1: estimate ITI
    all_events = [event.sample; event.value];
    is = find(all_events(2,:)==110);

    stimuli_times_block_1 = all_events(1,is);

    iti_duration_ms = diff(stimuli_times_block_1);  % in participants 24 and 28 the first number in the 'is' variable is 1, and when 'is-1' is used as an index, the index becomes 0, and an index cannot be 0

    % Binary feedback on reward acquisition was visible for 1900 [ ± 100] ms,
    figure; histogram(iti_duration_ms,20); set(gca,'FontSize',20)

    % assess how long ITI is after removing feedback display for max 2000ms
    iti_duration_ms(iti_duration_ms<3000) = nan;
    % Stim not preceeded by an outcome?
    iti_duration_ms = iti_duration_ms - 2000;
    figure; histogram(iti_duration_ms,20); set(gca,'FontSize',20)
    cc = [min(iti_duration_ms), nanmedian(iti_duration_ms), max(iti_duration_ms)];
    ...
    for i = 1:length(events_stimulus_1)
        if events_stimulus_1(2, i) ~= 110
            indices_to_remove(end+1) = i; % from all events
        end
    end
    ...
    event = ft_read_event(fif_filename2, 'chanindx', channel);    % change stimulus channel

    %% ITI duration check
    % all events
    all_events = [event.sample; event.value];

    if n == 22                                                 %  both blocks in zhanx_22 contain 110 as the stimulus trigger, because block_1 was recorded twice due to a technical error
        is2 = find(all_events(2,:)==110);
    else
        is2 = find(all_events(2,:)==40);
    end    
    ...
    % assess how long ITI is after removing feedback display for max 2000ms
    % replace not-valid ITI with nan

    iti2_duration_ms(iti2_duration_ms<3000) = nan;
    % Stim not preceeded by an outcome?

    iti2_duration_ms = iti2_duration_ms - 2000;
    figure; histogram(iti2_duration_ms,20); set(gca,'FontSize',20)
    cc2 = [min(iti2_duration_ms), nanmedian(iti2_duration_ms), max(iti2_duration_ms)];
    ...
    %@ tpeaks_tokeep contains latency of Tpeaks at n-1, n-2 that preceed stimuli
    tpeaks_tokeep = cat(2, nearest_previous_values_1, nearest_previous_values_2);

    %% Check Twave peaks latency values
    % @ latency_peaks includes LATENCY DIFFERENCE BETWEEN n-1, n-2, Tpeaks
    % preceding stimuli & stimuli
    load([fpath0,'latency_tpeaks.mat'])


    % These values are all in milliseconds because Fs = 1000 Hz
    % Check latency_peaks
    thr1 = 400;  % we only accept T wave peaks that precede the stimulus onset by more than thr1 milliseconds
    thr2 = 1200; % exclude trials when the T wave peak in latency_tpeaks(:,2) is more than thr2 farther apart from latency_tpeaks(:,1)
    ...
    %@ Check histogram: Tpeak at n-1 from stimulus
    figure;histogram(latency_tpeaks(ikeep,1),20); set(gca,'FontSize',20)

    %@ Check how far apart the Tpeak at n-2 is from stimulus, we don't want
    %this to extend back into the outcome display window, should not be longer
    %than ITI on the trial
    figure;histogram(latency_tpeaks(ikeep2,2),20); set(gca,'FontSize',20)
    ...
    iti_duration_ms_all = cat(2, iti_duration_ms, iti2_duration_ms);

    ikeep3 = find(latency_tpeaks(:,2)<=iti_duration_ms_all');
    ikeep2 = intersect(ikeep2, ikeep3);
    figure;histogram(latency_tpeaks(ikeep2,2),20); set(gca,'FontSize',20)
